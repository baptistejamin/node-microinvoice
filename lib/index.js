...
module.exports = class Microinvoice {
  ...
  generateLegal() {
    ...
    (this.options.data.invoice.legal || []).forEach((legal) => {
      ...
      this.setText(legal.value, {
        maxWidth: this.document.page.width - this.options.style.document.marginLeft * 2,
  ...
  prettyPrice(value) {
    ...
    if (!isNaN(value)) {
      prettifiedValue = Number(value).toFixed(2);
    }
  ...
  setText(text, options = {}) {
    ...
    const currentSectionHeight = this.document.heightOfString(text, {width: _maxWidth});
    if (currentSectionHeight + this.storage.cursor.y > this.document.page.maxY()) {
      this.document.addPage();
      this.setTextStyle(text, options);
